...
using MicrositeAmey.Membership.Code;
using MicrositeAmey.Membership.Models;
using Umbraco.Core;
using Umbraco.Core.Persistence.Querying;
using Umbraco.Web.Mvc;

namespace MicrositeAmey.Membership.Controllers.Surface
{
    [PluginController("AmeyMembership")]
    public class AuthSurfaceController : SurfaceController
    {
        ...
        public ActionResult RenderLogin()
        {
            ...
            {
                ReturnUrl = string.IsNullOrEmpty(HttpContext.Request["ReturnUrl"])
                    ? "/"
                    : HttpContext.Request["ReturnUrl"]
            };
        ...
        public ActionResult HandleLogin(LoginViewModel model)
        {
            ...
            try
            {
                ...
                if (System.Web.Security.Membership.ValidateUser(model.EmailAddress, model.Password))
                {
                    //Valid credentials
        ...
        {
            return PartialView("_ForgottenPassword", new ForgottenPasswordViewModel());
        }
                ...
                if (!string.IsNullOrEmpty(resetQueryString))
                {
                    ...
                    if (resetMember.Properties["resetGUID"].Value.ToString() == resetQueryString)
                    {
                        ...
                        if (currentTime.CompareTo(expiryTime) < 0)
                        {
                            ...
                            //ToDo: CA:Previous line not possible... see:
                            //https://our.umbraco.com/forum/extending-umbraco-and-using-the-api/85722-problem-updating-members-password-using-umbraco-version-761
                            //https://stackoverflow.com/questions/15003898/change-umbraco-member-password-with-out-knowing-old-password
                            // this only works because in web.config the membership provider attribute was set to:
                            ...
                            return Redirect(Constants.UrlLogin);
                        }
                        else
                        {
                    ...
                    else
                    {
                        ...
                        //Invalid GUID
                        ModelState.AddModelError("ResetPasswordForm.", "Unable to reset your password");
                        return CurrentUmbracoPage();
                    }
                }
                else
        ...
        public ActionResult HandleRegister(RegisterViewModel model)
        {
            ...
            try
            {
                ...
                var createMember = membershipService.CreateMember(model.EmailAddress, model.EmailAddress, model.Name, "MemberSTRUSE");

                //Set the verified email to false
                createMember.Properties["hasVerifiedEmail"].Value = false;

                //Set the profile URL to be the GUID, so they have a unique profile ID, until they go to set it
                createMember.Properties["profileURL"].Value = Guid.NewGuid();
        ...
        {
            if (string.IsNullOrWhiteSpace(verifyGUID))
            {
            ...
            if (findMember != null)
            {
                ...
                //Add role that allows member to view member area
                membershipService.AssignRole(findMember.Id, "Signed up for alerts");
